...
export function firmsRowsToFires(rows: Record<string, string>[]): FirmsFire[] {
  ...
  return clusterPoints(points).map((group) => {
    ...
    return {
      ...
      notes:
        'نقطة حرارية رُصدت آلياً عبر الأقمار الصناعية في هذا التوقيت. ' +
        'لا تعني بالضرورة وجود حريق مستمر الآن، وقد تكون مصدراً حرارياً آخر. ' +
        'الولاية مُقدَّرة من الإحداثيات.',
    }
  })
}
